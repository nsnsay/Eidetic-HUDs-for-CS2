export default {
  ...
  computed: {
    isInstalledVersionOutdated() {
      if (!this.installedVersionString || !this.latestVersionString)
        return null;
  ...
  methods: {
    ...
    async checkForUpdates() {
      ...
      try {
        const response = await fetch(
          'https://api.github.com/repos/drweissbrot/cs-hud/releases/latest'
        );
    ...
    parseVersionString(str) {
      const isValid =
        typeof str === 'string' && /^v?\d+(\.\d+)+(-.+)?$/i.test(str);
      if (!isValid) return;
