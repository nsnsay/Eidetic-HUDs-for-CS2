...
const parsePlayerWeapons = (player) =>
  Object.values(player.weapons)
    .flatMap((weapon) => {
      ...
      if (parsed.name === 'weapon_flashbang' && parsed.ammoReserve >= 2)
        return [parsed, { ...parsed, isActive: false }];
...
export const parsePlayers = () => {
  ...
  for (const [steam64Id, player] of Object.entries(gsiState.allplayers)) {
    ...
    const kdRatio =
      player.match_stats?.kills / (player.match_stats?.deaths || 1);
...
const getAdr = (steam64Id) => {
  ...
  const roundDamages = Object.entries(
