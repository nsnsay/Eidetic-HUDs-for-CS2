...
import {
  getOverriddenTeamName,
  getTeamNameOverrides,
} from '/hud/gsi/helpers/team-name-overrides.js';
...
const makeTeam = (side, gsiTeamObject, teamNameOverrides) => {
  ...
  const overriddenTeamName = getOverriddenTeamName(
    teamNameOverrides,
    teamMembers
  );
  ...
  const team = {
    ...
    name:
      overriddenTeamName || gsiTeamObject.name || getFallbackNameFromSide(side),
    players: teamMembers,
