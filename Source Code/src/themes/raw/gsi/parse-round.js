...
export const parseRound = () => {
  ...
  const roundNumber =
    gsiState.map.round + 1 - Number(gsiState.phase_countdowns.phase === 'over');
  const isOvertime = roundNumber > maxrounds;

  const roundsInOvertimes = isOvertime ? roundNumber - maxrounds : 0;
  const overtimeNumber = isOvertime
    ? Math.floor((roundsInOvertimes - 1) / overtimeMaxrounds) + 1
    : 0;
  const matchPointAtScore = isOvertime
    ? maxrounds / 2 +
      overtimeMaxrounds / 2 +
      (overtimeNumber - 1) * (overtimeMaxrounds / 2)
    : Math.floor(maxrounds / 2);
  ...
  return {
    ...
    roundNumberInCurrentOvertime: isOvertime
      ? roundNumber -
        maxrounds -
        (overtimeNumber - 1) * overtimeMaxrounds -
        Number(gsiState.phase_countdowns.phase === 'over')
      : 0,
