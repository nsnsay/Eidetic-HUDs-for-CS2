...
export const parseRounds = () => {
  ...
  const roundNumberOffset =
    gsiState.map.round <= mpMaxrounds
      ? 0
      : mpMaxrounds +
        Math.floor((gsiState.map.round - mpMaxrounds) / mpOvertimeMaxrounds) *
          mpOvertimeMaxrounds;

  return Object.entries(gsiState.map.round_wins || {}).map(
    ([roundNumber, reason]) => {
