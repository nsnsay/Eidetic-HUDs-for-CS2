		...
		matches() {
			...
			for (const mapNumber of this.seriesMapNumbers) {
				...
				maps.push({
					...
					mapImageUrl: mapName ? `/hud/img/maps/${mapName}.webp` : `/hud/img/maps/random.webp`, // 如果mapName为null，使用默认图片
					mapName: mapName ? this.formatMapName(mapName) : "", // 如果mapName为null，使用默认名称
